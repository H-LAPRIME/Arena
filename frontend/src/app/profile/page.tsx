          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "24px" }}>
            ...
            {user.is_lord ? (
              ...
              <div className="card" style={{ padding: "24px", textAlign: "center", border: "1px dashed var(--border)" }}>
                <div className="card-bg-watermark"><TrophyIcon /></div>
                <div style={{ color: "var(--text-muted)", marginBottom: "12px", display: "flex" }}><CrownIcon /></div>
                <p style={{ fontSize: "14px", fontWeight: 700 }}>Lord Status</p>
                <p style={{ fontSize: "12px", color: "var(--text-muted)", marginBottom: "16px" }}>Win 3 titles in any league series</p>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "8px", textAlign: "center" }}>
                  {Object.entries((myLeagues.reduce((acc: any, l: any) => {
                    const base = l.name.replace(/ V\d+$/i, "").trim();
                    if (!acc[base]) acc[base] = [];
                    acc[base].push(l);
                    return acc;
                  }, {}) as any)).map(([baseName, leagues]: [string, any]) => {
                    const titlesCount = leagues.filter((l: any) => l.champion_id === user.id).length;
                    return (
                      <div key={baseName} style={{ background: "rgba(255,255,255,0.03)", padding: "8px", borderRadius: "8px", border: "1px solid var(--border)" }}>
                        <div style={{ fontSize: "18px", fontWeight: 800, color: titlesCount >= 3 ? "var(--gold)" : "var(--accent)" }}>{titlesCount}</div>
                        <div style={{ fontSize: "10px", color: "var(--text-muted)", marginTop: "2px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{baseName}</div>
                      </div>
                    );
                  })}
                </div>
          ...
          <div className="card">
            ...
            <div style={{ padding: "0" }}>
              {myLeagues.length === 0 ? (
                ...
                <div className="achievement-list" style={{ display: "flex", flexDirection: "column", gap: "16px", padding: "16px" }}>
                  {Object.entries(myLeagues.reduce((acc: any, l: any) => {
                    const base = l.name.replace(/ V\d+$/i, "").trim();
                    if (!acc[base]) acc[base] = [];
                    acc[base].push(l);
                    return acc;
                  }, {})).map(([baseName, leagues]: [string, any]) => {
                    const titlesCount = leagues.filter((l: any) => l.champion_id === user.id).length;
                    const isLord = titlesCount >= 3;
                    const activeLeague = leagues.find((l: any) => l.status === "active" || l.status === "pending");

                    return (
                      <div key={baseName} style={{ background: "#ffffff", border: "1px solid var(--border)", borderRadius: "12px", padding: "16px" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px", paddingBottom: "12px", borderBottom: "1px solid var(--border)" }}>
                          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                            ...
                            <div>
                              <div style={{ fontWeight: 800, fontSize: "16px" }}>{baseName}</div>
                              {isLord ? (
                                <div style={{ color: "var(--gold)", fontWeight: 800, fontSize: "12px", marginTop: "4px", display: "flex", alignItems: "center", gap: "4px" }}><CrownIcon /> LORD OF THE GAME • {titlesCount}/3 Titles</div>
                              ) : (
                                <div style={{ fontSize: "12px", color: "var(--text-muted)", display: "flex", alignItems: "center", gap: "4px", flexWrap: "wrap" }}><TrophyIcon /> {titlesCount}/3 Titles • {activeLeague ? (activeLeague.is_auto_created ? <><BallIcon /> Auto-started</> : "Active") : "Completed Series"}</div>
                              )}
                            </div>
                          </div>
                          <div style={{ display: "flex", alignItems: "center", gap: "6px", background: "rgba(255, 215, 0, 0.1)", padding: "6px 12px", borderRadius: "8px", border: "1px solid rgba(255, 215, 0, 0.2)" }}>
                            <span style={{ display: "flex" }}><TrophyIcon /></span>
                            <span style={{ fontWeight: 700, color: "var(--gold)", fontSize: "14px" }}>{titlesCount}</span>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                          {leagues.map((l: any) => {
                            ...
                            return (
                              <div key={l.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "#ffffff", border: "1px solid var(--border)", padding: "12px", borderRadius: "8px" }}>
                                <div>
                                  ...
                                  <div style={{ fontSize: "11px", color: "var(--text-muted)", display: "flex", alignItems: "center", gap: "4px" }}>{isChamp ? <>Champion <TrophyIcon /></> : "Participant"}</div>
                                </div>
                                <div style={{ display: "flex", gap: "8px" }}>
                                  {isChamp && (
                                    <button onClick={() => certificatesApi.downloadTitle(l.id, l.name)} className="btn btn-sm btn-gold" style={{ fontSize: "11px", padding: "4px 8px" }}>Badge</button>
                                  )}
                                  <button onClick={() => certificatesApi.downloadReport(l.id, l.name, user.username)} className="btn btn-sm btn-secondary" style={{ fontSize: "11px", padding: "4px 8px" }}>Report</button>
                                </div>
                              </div>
                            );
                          })}
