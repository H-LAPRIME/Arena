    ...
    <div className="page-container">
      <div className="page-header">
        ...
        <p className="page-subtitle">
          {league ? `${league.name} - ${league.status === "completed" ? "Terminated season" : league.status === "active" ? "Ongoing season" : "Pending season"}` : "No league selected"}
        </p>
      </div>
      ...
      {selectedLeagueId && !loadingMatches ? (
        <div className="match-list">
          {matches.map((m: any) => {
            ...
            const canSubmitResult = league?.status === "active" && isMyMatch && m.status === "pending";
            return (
              <div 
                key={m.id} 
                className={`match-card ${m.status} ${isHome ? "home-match" : ""} ${isAway ? "away-match" : ""}`}
                onClick={() => canSubmitResult && router.push(`/claims?matchId=${m.id}`)}
                style={{ cursor: canSubmitResult ? "pointer" : "default" }}
              >
                {isHome && (
