...
export default function AdminPage() {
  ...
  async function handleNewSeason(id: string) {
    if(!confirm("Create a new season from this completed league?")) return;
    try { 
      await leaguesApi.newSeason(id); 
      setMsg("success:New season created!"); 
      loadAll(); 
    } catch (err: any) { 
  ...
  function getNextSeasonName(name: string) {
    const match = name.match(/ V(\d+)$/);
    return match ? name.replace(/ V\d+$/, ` V${parseInt(match[1]) + 1}`) : `${name} V2`;
  }

  function getBaseSeasonName(name: string) {
    return name.replace(/ V\d+$/i, "").trim();
  }

  function getSeasonNumber(name: string) {
    const match = name.match(/ V(\d+)$/i);
    return match ? parseInt(match[1], 10) : 1;
  }

  function getGroupedSeasons() {
    const filtered = seasons.filter((s: any) =>
      s.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      s.join_code.toLowerCase().includes(searchTerm.toLowerCase())
    );
    const grouped: Record<string, any[]> = {};
    filtered.forEach((season: any) => {
      const base = getBaseSeasonName(season.name);
      if (!grouped[base]) grouped[base] = [];
      grouped[base].push(season);
    });
    return Object.entries(grouped)
      .sort(([a], [b]) => a.localeCompare(b))
      .map(([baseName, items]) => ({
        baseName,
        seasons: items.sort((a: any, b: any) => getSeasonNumber(b.name) - getSeasonNumber(a.name)),
      }));
  }
    ...
    <div className="page-container">
      ...
      {tab === "seasons" && (
        <div className="card" style={{ padding: 0, overflow: "hidden", maxHeight: "750px", display: "flex", flexDirection: "column" }}>
          ...
          <div className="admin-table-container" style={{ flex: 1, overflowY: "auto", display: "grid", gap: "18px" }}>
            {getGroupedSeasons().map((group, index) => {
              const isOpen = !!expandedSeasonGroups[group.baseName];
              const hasActiveSeason = group.seasons.some((s: any) => s.status === "active");
              const gradients = [
                "linear-gradient(135deg, rgba(37, 99, 235, 0.12), rgba(34, 197, 94, 0.07))",
                "linear-gradient(135deg, rgba(217, 119, 6, 0.12), rgba(37, 99, 235, 0.06))",
                "linear-gradient(135deg, rgba(124, 58, 237, 0.11), rgba(20, 184, 166, 0.06))",
                "linear-gradient(135deg, rgba(34, 197, 94, 0.11), rgba(239, 68, 68, 0.05))",
              ];
              return (
              <div key={group.baseName} className="table-container" style={{ borderRadius: "16px", overflow: "hidden", border: "1px solid var(--border)" }}>
                <button
                  type="button"
                  onClick={() => setExpandedSeasonGroups(prev => ({ ...prev, [group.baseName]: !prev[group.baseName] }))}
                  style={{
                    width: "100%",
                    border: 0,
                ...
                >
                  <h3 style={{ margin: 0, fontSize: "17px", fontWeight: 900 }}>{group.baseName}</h3>
                  <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                    {hasActiveSeason && <span className="badge badge-green">active</span>}
                    <button
                      type="button"
                      ...
                      onClick={async (e) => {
                        e.stopPropagation();
                        if (!confirm(`DELETE ENTIRE LEAGUE "${group.baseName}" and ALL its seasons (${group.seasons.length})? This will delete ALL matches, standings, claims, and data. This is IRREVERSIBLE!`)) return;
                        try {
                          for (const s of group.seasons) {
                            await leaguesApi.delete(s.id);
                          }
                          setMsg(`success:Entire league "${group.baseName}" deleted (${group.seasons.length} seasons).`);
                          loadAll();
                        } catch (err: any) {
                          setMsg("error:" + err.message);
                        }
                    ...
                    <span className="badge">{isOpen ? "Hide seasons" : "View seasons"}</span>
                  </div>
                </button>
                {isOpen && (
                <table className="admin-table">
                  <thead>
                    <tr><th>Season</th><th>Code</th><th>Members</th><th>Status</th><th>Champion Info</th><th>Actions</th></tr>
                  </thead>
                  <tbody>
                    {group.seasons.map((s: any) => (
                    <tr key={s.id}>
                      <td>
                        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                      ...
                      <td>
                        {s.status === "completed" && s.champion_trophies !== undefined ? (
                          <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                            <span style={{ fontSize: "12px", fontWeight: 600 }}>
                              <TrophyIcon /> {s.champion_trophies}/3
                            </span>
                            {s.champion_trophies >= 3 && (
                              <span style={{ fontSize: "11px", color: "gold", fontWeight: 700, display: "inline-flex", alignItems: "center", gap: "4px" }}><CrownIcon /> LORD</span>
                            )}
                          </div>
                        ) : (
                      ...
                      <td>
                        ...
                        {s.status === "completed" && s.champion_trophies !== undefined && s.champion_trophies < 3 && !seasonExists(getNextSeasonName(s.name)) && (
                          <button onClick={() => handleNewSeason(s.id)} className="btn btn-sm btn-green" style={{ marginRight: "4px" }}>New Season (V+1)</button>
                        )}
                        {s.status === "completed" && s.champion_trophies !== undefined && s.champion_trophies < 3 && seasonExists(getNextSeasonName(s.name)) && (
                          <button disabled className="btn btn-sm btn-green" style={{ marginRight: "4px", opacity: 0.5, cursor: "not-allowed" }} title="Next season already exists">Next Season Exists</button>
                        )}
                        {s.status === "completed" && s.champion_trophies !== undefined && s.champion_trophies >= 3 && (
                          <button disabled className="btn btn-sm btn-green" style={{ marginRight: "4px", opacity: 0.5, cursor: "not-allowed" }} title="Series ended - Champion is LORD">New Season (Series Ended)</button>
                        )}
        ...
        <div className="modal-overlay">
          <div className="modal-content card" style={{ width: "500px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
              ...
              <button onClick={() => setIsMembersModalOpen(false)} className="btn btn-sm" style={{ minWidth: "auto", padding: "4px 8px" }}><XIcon /></button>
            </div>
                ...
                <div key={m.id} style={{ display: "flex", alignItems: "center", gap: "12px", background: "rgba(255,255,255,0.03)", padding: "12px", borderRadius: "10px", border: "1px solid var(--border)" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "4px", background: "rgba(255, 215, 0, 0.1)", padding: "4px 8px", borderRadius: "6px", border: "1px solid rgba(255, 215, 0, 0.2)" }}>
                      <span style={{ color: "gold", display: "flex" }}><TrophyIcon /></span>
