...
export default function StatsPage() {
  ...
  async function loadLeagueStats(lid?: string) {
    setLoadingData(true);
    try {
      const [r, leagueData] = await Promise.all([
        statsApi.records(lid),
        lid ? leaguesApi.getOne(lid) : Promise.resolve(null)
      ]);
      setRecords(r);
    ...
    <div className="page-container">
      <div className="page-header">
        ...
        <p className="page-subtitle">
          {selectedLeague ? `${selectedLeague.name} - ${selectedLeague.status === "completed" ? "Terminated season" : selectedLeague.status === "active" ? "Ongoing season" : "Pending season"}` : "Global records, matches and analysis"}
        </p>
      </div>
      ...
      {records?.player_stats && (
        <div className="card" style={{ marginBottom: "32px", padding: 0, overflow: "hidden" }}>
          ...
          <div className="card-header" style={{ padding: "24px 24px 0 24px" }}><span className="card-title"><TrophyIcon /> {selectedLeague ? "Season Ranking" : "Global Ranking (All Seasons)"}</span></div>
