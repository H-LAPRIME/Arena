...
@router.get("/check-login-email")
def check_login_email(email: str, db: Session = Depends(get_db)):
    user = db.query(User).filter(User.email == email).first()
    if not user:
        raise HTTPException(status_code=404, detail="Email not found")
    if not user.is_active:
        raise HTTPException(status_code=403, detail="Account deactivated")
    return {"exists": True, "username": user.username, "email": user.email}
...
def check_username(username: str, db: Session = Depends(get_db)):
    ...
    exists = db.query(User).filter(User.username == clean_username).first() is not None
    if exists:
        raise HTTPException(status_code=409, detail="Username already taken")
    return {"available": True, "username": clean_username}


@router.get("/check-signup-email")
def check_signup_email(email: str, db: Session = Depends(get_db)):
