...
from app.services.league_lifecycle import complete_active_league, count_series_titles
...
def _league_response(league: League, db: Session) -> LeagueResponse:
    ...
    if league.champion_id:
        r.champion_trophies = count_series_titles(db, league.champion_id, league.name)
...
def delete_league(league_id: str, admin: User = Depends(require_admin), db: Session = Depends(get_db)):
    ...
    # Delete claims linked to this league
    db.query(ResultClaim).filter(ResultClaim.league_id == league_id).delete(synchronize_session=False)
    # Delete titles linked to this league
    db.query(Title).filter(Title.league_id == league_id).delete(synchronize_session=False)
    # Delete matches linked to this league
    db.query(Match).filter(Match.league_id == league_id).delete(synchronize_session=False)
    # Delete standings linked to this league
    db.query(Standing).filter(Standing.league_id == league_id).delete(synchronize_session=False)
    # Delete members linked to this league
    db.query(LeagueMember).filter(LeagueMember.league_id == league_id).delete(synchronize_session=False)
    
    db.delete(league)
    db.commit()
    return {"message": "League and all related data deleted"}


@router.post("/leagues/{league_id}/new-season", response_model=LeagueResponse)
def new_season(league_id: str, admin: User = Depends(require_admin), db: Session = Depends(get_db)):
    old_league = db.query(League).filter(League.id == league_id).first()
    if not old_league:
        raise HTTPException(status_code=404, detail="League not found")
    if old_league.status != "completed":
        raise HTTPException(status_code=400, detail="Only completed leagues can have a new season")
    if old_league.champion_id and count_series_titles(db, old_league.champion_id, old_league.name) >= 3:
        raise HTTPException(status_code=400, detail="This league series has ended because the champion reached 3 titles")

    # Calculate new name
    match = re.search(r' V(\d+)$', old_league.name)
    if match:
        v_num = int(match.group(1))
        new_name = re.sub(r' V\d+$', f' V{v_num + 1}', old_league.name)
    else:
        new_name = f"{old_league.name} V2"
    if db.query(League).filter(League.name == new_name).first():
        raise HTTPException(status_code=400, detail=f"Season '{new_name}' already exists")
    ...
    new_league = League(
        ...
        join_code=join_code,
        max_members=old_league.max_members,
        created_by=admin.id,
        status="pending",
    )
    db.add(new_league)
    ...
    # Copy members
    old_members = db.query(LeagueMember).filter(LeagueMember.league_id == old_league.id).all()
    ...
    return _league_response(new_league, db)
...
async def complete_league(league_id: str, admin: User = Depends(require_admin), db: Session = Depends(get_db)):
    league = db.query(League).filter(League.id == league_id).first()
    if not league:
        raise HTTPException(status_code=404, detail="League not found")
    if league.status != "active":
        raise HTTPException(status_code=400, detail="League is not active")
    completed = await complete_active_league(db, league)
    if not completed:
        raise HTTPException(status_code=400, detail="League has no standings to resolve")
    db.refresh(league)
    return _league_response(league, db)
...
def reject_claim(
    ...
    notif = Notification(
        ...
        notif_data=json.dumps({"league_id": claim.league_id, "match_id": claim.match_id}),
    )
    db.add(notif)
    db.commit()
