...
def resolve_champion(db: Session, league: League) -> None:
    ...
    # 2. Check for Lord of the Game (3 trophies in this specific league)
    import re
    ...
    titles_in_league = sum(1 for l in leagues_won if re.sub(r' V\d+$', '', l.name).strip() == base_name)

    if titles_in_league >= 3 and not champion.is_lord:
